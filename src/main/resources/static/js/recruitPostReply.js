    ...
    function updateReplyList(data) {
        ...
        for (let r of data) {
            str += '<div class="card my-2">'
                ...
                + '<p style="font-size: 12px; color:gray; margin:0px;"> 작성 시간: ' + r.createdTime + '</p>'
                //+ '<p style="font-size: 12px; color:gray;"> 수정 시간: ' + r.modifiedTime+ '</p>'
                + '</div>';
            // 댓글 작성자 아이디와 로그인 사용자 아이디가 같을 때만 "수정" 보여주기.
            if (r.writer == loginUser) {
                str += '<div class="card-footer" style="background-color: white; border:none; padding-top:0px; padding-bottom:0px;">'
                    + `<button type="button" style=" padding:5px; margin-right:10px; border:none; color:blue;" class="btnModifies btn" data-rid="${r.replyId}">수정</button>`;
            };
            str += `<button type="button" style="padding:5px; width:55px; color=black; border:none; "class="rreplyBtn btn" data-rid="${r.replyId}">답글</button>`
                + '</div>'
                + '<div id="repliesTwo" style="margin-right: 60px; margin-left: 60px;"></div>'
                + '</div>';
        }
        ...
        // 대댓글 작업중
        const rreplyBtn = document.querySelectorAll('.rreplyBtn');
        rreplyBtn.forEach(btn=>{
            btn.addEventListener('click', getRreply);
        })
    }
